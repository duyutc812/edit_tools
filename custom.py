...
class LeCartCustom(LeBasecart):
	def display_config_source(self):
		...
		self._notice['src']['support']['country_map'] = True
	...
	def display_import_source(self):
		...
		queries = {
			...
			# 	'type': 'select',
			# 	'query': "SELECT COUNT(1) AS count FROM  _DBPRF_categories_description WHERE categories_id > " + to_str(self._notice['process']['orders']['id_src']),
			# },
		}
	...
	def clear_target_categoriprepare_importes(self):
		next_clear = {
			'result': 'process',
	...
	def get_categories_main_export(self):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM  _DBPRF_categories WHERE categories_id > " + to_str(id_src) + " ORDER BY categories_id ASC LIMIT " + to_str(limit)
		}
	...
	def get_categories_ext_export(self, categories):
		...
		categories_ext_queries = {
			...
			'products_to_categories': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_products_to_categories WHERE categories_id IN " + self.list_to_in_condition(
					category_ids)
			},
		}
		categories_ext = self.select_multiple_data_connector(categories_ext_queries)
		lang_ids = duplicate_field_value_from_list(categories_ext['data']['categories_description'], 'language_id')
		if not categories_ext or categories_ext['result'] != 'success':
			return response_warning()
		categories_ext_rel_queries = {
			'languages': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_languages WHERE languages_id IN " + self.list_to_in_condition(
					lang_ids)
			},
		}
		categories_ext_rel = self.get_connector_data(url_query, {'serialize': True, 'query': json.dumps(categories_ext_rel_queries)})
	...
	def convert_category_export(self, category, categories_ext):
		...
		# img = get_row_from_list_by_field(category, 'categories_id', category['categories_id'])
		if category['categories_image']:
			category_data['thumb_image']['url'] = 'http://cdn.shopify.com/s/files/1/0525/4542/1506/collections/'
		...
		if category_description:
			category_data['description'] = category_description['categories_name'] if category_description['categories_name'] != '' else ''
		else:
			category_data['description'] = ''
		category_data['name'] = category_description['categories_name']
		category_data['meta_title'] = ''
		category_data['meta_keyword'] = ''
		# category_lang = self.construct_category_lang()
		cate_lang = get_row_from_list_by_field(categories_ext['data']['languages'], 'languages_id', category_description['language_id'])
		if cate_lang:
			# category_lang['name'] =cate_lang['name']
			category_data['languages'] = cate_lang
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			...
			'products_to_categories': {
				'type': "select",
				'query': "SELECT * FROM  _DBPRF_products_to_categories WHERE products_id IN " + product_id_con, # + " ORDER BY disp_order",
			},
			'orders_products': {
				'type': "select",
	...
	def convert_product_export(self, product, products_ext):
		...
		# product_data['created_at'] = datetime.fromtimestamp(to_int(product['products_date_added'])) if product['products_date_added'] else get_current_time()
		# product_data['updated_at'] = datetime.fromtimestamp(to_int(product['products_last_modified'])) if to_int(product['products_last_modified']) else product_data['created_at']
		product_data['created_at'] = product['products_date_added']
		product_data['updated_at'] = product['products_last_modified']

		product_detail = get_row_from_list_by_field(products_ext_data['products_description'], 'products_id', product['products_id'])
		product_data['name'] = html.unescape(product_detail['products_name'])
		product_data['description'] = html.unescape(product_detail['products_description'])
		# product_categories = get_list_from_list_by_field(products_ext_data['products_meta'], 'product_id', product['id'])
		...
		# product_data['tags'] = product_description_def['tag']
		url_product_image = 'https://cdn.shopify.com/s/files/1/0525/4542/1506/products/'
		...
		# product_images = get_list_from_list_by_field(product, 'product_id',product['products_id'])
		# if product_images:
		# 	if check_thumbnail:
		# 		for product_image in product_images:
		# 			image_data = get_row_from_list_by_field(product, 'products_image', product_image['image_id'])
		...
		if product_lang:
			...
			product_lang['description'] = language_id['code']
			product_data['languages'] = product_lang
	...
	def get_customers_ext_export(self, customers):
		...
		customers_ext_rel_queries = {
			...
			# 'currencies': {
			# 	'type': 'select',
			# 	'query': "SELECT * FROM  _DBPRF_currencies WHERE currencies_id  IN " + self.list_to_in_condition(zone_ids),
			# },
		}
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		customer_data['username'] = customer['customers_firstname'] + customer['customers_lastname']
	...
	def get_orders_ext_export(self, orders):
		...
		currency_id = duplicate_field_value_from_list(orders['data'], 'currency')
		...
		bil_state = duplicate_field_value_from_list(orders['data'], 'billing_state')
		...
		orders_ext_queries = {
			...
			'customers': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_customers WHERE customers_id  IN " + self.list_to_in_condition(cus_ids)
			},
			...
			'currencies': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_currencies WHERE code IN " + self.list_to_in_condition(currency_id)
			},
			'zones': {
				'type': 'select',
	...
	def convert_order_export(self, order, orders_ext):
		...
		if ot_tax:
			...
			order_data['tax']['percent'] = ot_tax['text']
			if ot_subtotal:
		...
		# customer_address['first_name'] = order['billing_name']
		...
		customer_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_name', order['billing_country'])
		if customer_country:
		...
		customer_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'zone_code', order['billing_state'])
		if customer_state:
		...
		order_billing['last_name'] = order['billing_name']
		...
		billing_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_name', order['billing_country'])
		if billing_country:
			order_billing['country']['id'] = billing_country['countries_id']
			order_billing['country']['code'] = billing_country['countries_iso_code_2']
			order_billing['country']['country_code'] = billing_country['countries_iso_code_3']
			order_billing['country']['name'] = billing_country['countries_name']

		billing_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'zone_code', order['billing_state'])
		if billing_state:
		...
		order_delivery['last_name'] = order['delivery_name']
		...
		delivery_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_name', order['delivery_country'])
		if delivery_country:
			order_delivery['country']['id'] = delivery_country['countries_id']
			order_delivery['country']['code'] = delivery_country['countries_iso_code_2']
			order_delivery['country']['country_code'] = delivery_country['countries_iso_code_3']
			order_delivery['country']['name'] = delivery_country['countries_name']
		delivery_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'zone_code', order['delivery_state'])
		if delivery_state:
