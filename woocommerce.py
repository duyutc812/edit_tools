...
class LeCartWoocommerce(LeCartWordpress):
	...
	def category_import(self, convert, category, categories_ext):
		...
		category_data_query = self.create_insert_query_connector('wp_terms', terms)
		cate_id = self.import_data_connector(category_data_query, 'category')
		if cate_id:
			self.insert_map(self.TYPE_CATEGORY, convert['id'], cate_id)
		else:
		...
		query = self.create_insert_query_connector('wp_term_taxonomy', term_taxonomy)
		term_taxonomy_id = self.import_data_connector(query, 'category')


		category_lookup = {
			'category_tree_id': cate_id,
			'category_id': cate_id,
		}
		query = self.create_insert_query_connector('wp_wc_category_lookup', category_lookup)
		category_lookup_id = self.import_data_connector(query, 'category')
		...
		for key,val in cate_termmeta_value.items():
			cate_termmeta = {
				'term_id': cate_id,
				'meta_key': key,
				'meta_value': val,
			}
			query = self.create_insert_query_connector('wp_termmeta', cate_termmeta)
			self.import_category_data_connector(query, True, convert['id'])
		# if cate_id:
	...
	def product_import(self, convert, product, products_ext):
		...
		posts_data_query = self.create_insert_query_connector('wp_posts', posts)
	...
	def customer_import(self, convert, customer, customers_ext):
		...
		customer_data_query = self.create_insert_query_connector('wp_users', users)
		...
		if customer_id:
			for key, value in user_meta_value.items():
				...
				user_meta_data_query = self.create_insert_query_connector('wp_usermeta', user_meta)
		...
		customer_lookup_query = self.create_insert_query_connector('wp_wc_customer_lookup', customer_lookup)
		self.import_customer_data_connector(customer_lookup_query, True, 'user')
		self.insert_map(self.TYPE_CUSTOMER, convert['id'], customer_id, )
		return response_success(0)
